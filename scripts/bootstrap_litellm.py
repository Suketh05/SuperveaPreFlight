"""One-time bootstrap: insert the LiteLLM gateway row and run the first
catalogue sync so there are LiteLLM routes in the DB before you hit the
API. Run this once after applying the migration.

LiteLLM's "gateway" here is really just its public, static model/pricing
catalog JSON (see adapters/litellm_adapter.py's docstring) — no LiteLLM
proxy server deployment is required for this to sync real routes.
...
from app.preflight.adapters.litellm_adapter import DEFAULT_CATALOG_URL
...
async def main() -> None:
    ...
    async with pool.acquire() as conn:
        existing = await conn.fetchrow(FIND_GATEWAY_SQL, "LiteLLM")
        if existing:
    ...
    print("Running first catalogue sync (this fetches LiteLLM's real public catalog)...")
