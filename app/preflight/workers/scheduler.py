...
async def run_catalogue_sync_once(db_pool: asyncpg.Pool, gateway_row: dict) -> int:
    ...
    async with db_pool.acquire() as conn:
        normalizer = RouteNormalizer(conn)
...
async def run_catalogue_sync_loop(
    db_pool: asyncpg.Pool, gateway_rows: list[dict], interval_seconds: int = 900
) -> None:
    """Runs forever. Cancel the task to stop (e.g. on app shutdown)."""
    while True:
        for gateway_row in gateway_rows:
            try:
                await run_catalogue_sync_once(db_pool, gateway_row)
            except Exception:
